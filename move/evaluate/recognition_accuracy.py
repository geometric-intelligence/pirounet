...
if qualit_analysis:
    ...
    count_no_gravity = np.count_nonzero(dance_qual == 2) / (
        225 - np.count_nonzero(dance_qual == 0)
    )
    ...
    count_jumpy = np.count_nonzero(dance_qual == 3) / (
        225 - np.count_nonzero(dance_qual == 0)
    )
...
for i in range(len(all_labels_mathilde + 1)):
    ...
    if row_m[2] != 4.0:  # if there is effort label
...
conf_mat = confusion_matrix(labels_ln, labels_m, normalize="true")
