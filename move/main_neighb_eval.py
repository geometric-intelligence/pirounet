...
grid_xs = [np.arange(z_min, z_max, step_size[0]), np.arange(z_min, z_max, step_size[1]), np.arange(z_min, z_max, step_size[2])]
grid_ys = [- np.arange(z_min, z_max, step_size[0]), - np.arange(z_min, z_max, step_size[1]), - np.arange(z_min, z_max, step_size[2])]
n_xs = len(grid_xs[0])
n_ys = len(grid_ys[0])
...
for y in range(default_config.label_dim):
    ...
    for i, y_coord in enumerate(grid_ys[y]):
        for j, x_coord in enumerate(grid_xs[y]):
            if density[y][i, j] > density_thresh[y] and count[y, i, j] > dances_per_tile[y]:
...
for i in range(stat_sampling_size):
    ...
    for y in range(default_config.label_dim):
        ...
        for i in range(num_gen_lab):
            ...
            z_within_tile = torch.tensor(z).reshape(1, -1).to(default_config.device).float()

            x_create = model.sample(z_within_tile, onehot_encoder(0).reshape((1, 3)).to(default_config.device))
            x_create_formatted = x_create[0].reshape((default_config.seq_len, -1, 3))

            one_label_seq = np.append(one_label_seq, x_create_formatted.cpu().data.numpy()) # shape [n_test, 40, 53, 3]
            
        blind_sequences = np.append(blind_sequences, one_label_seq)

        blind_sequences = np.array(blind_sequences).reshape(-1, default_config.seq_len, 53, 3 )
    ...
    # ACTIVATIONS
    _, gen_activations = fid_classifier_model.forward(torch.tensor(set_of_blind_sequences_shuffled).to(default_config.device))
...
plt.savefig(f'evaluate/log_files/latent_space_{default_config.load_from_checkpoint}_PC_neighbour.png')
...
plt.savefig(f'evaluate/log_files/latent_space_{default_config.load_from_checkpoint}_neighbour_index.png')
...
plt.savefig(f'evaluate/log_files/neighb/latent_space_{default_config.load_from_checkpoint}_neighbour_effort.png')
