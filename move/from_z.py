...
z_0_mean = torch.tensor(np.median(z_0, axis = 0)).to(config.device)
z_1_mean = torch.tensor(np.median(z_1, axis = 0)).to(config.device)
z_2_mean = torch.tensor(np.median(z_2, axis = 0)).to(config.device)
...
for y in range(config.label_dim):
    ...
    filepath_for_artifacts = os.path.join(os.path.abspath(os.getcwd()), "evaluate/z_median/" + config.load_from_checkpoint)
    if exists(filepath_for_artifacts) is False:
        os.mkdir(filepath_for_artifacts)

    name = f"avg_dance_{y}.gif"
...
for y in range(config.label_dim):
    ...
    for i in range(n_radius_randoms):
        z_within_radius = torch.normal(zs[y], zstd_s[y]).to(config.device)
        ...
        name = f"around_{y}avg_dance{i}.gif"
