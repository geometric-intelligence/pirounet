...
class LstmEncoder(torch.nn.Module):
  def __init__(self, n_layers=2, input_features=3*53, h_features_loop=32, latent_dim=32):
    super().__init__()
    ...
    self.mean_block = torch.nn.Linear(
      h_features_loop, latent_dim)
    self.logvar_block = torch.nn.Linear(
      h_features_loop, latent_dim)
  ...
  def forward(self, inputs):
    ...
    print('done layer lstm1. It returned h1 of shape {} and h1_T of shape{}'.format(h1.shape, h1_T.shape))
    ...
    for i in range(self.n_layers-1):
        ...
        print('done layer lstm 2. lstm2 returns h2 of shape {} and h2_T of shape {}'.format(h2.shape, h2_T.shape))
...
class LstmDecoder(torch.nn.Module):
    def __init__(self, n_layers=2, output_features=3*53, h_features_loop=32, latent_dim=32, seq_len=128):
        super().__init__()
...
class LstmVAE(torch.nn.Module):
  ...
  def elbo(self, x_in, x_out, z, q_param, p_param=None):
      print('x_in has shape {} and x_out has shape {}'.format(x_in.shape, x_out.shape))
